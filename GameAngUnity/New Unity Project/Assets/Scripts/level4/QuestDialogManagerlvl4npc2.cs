...
public class QuestDialogManagerlvl4npc2 : MonoBehaviour {
	...
	void Update () {
		...
		if (dialogActive && Input.GetKeyDown (KeyCode.Alpha2)) {
		...
		}else if(dialogActive && Input.GetKeyDown (KeyCode.Alpha3)){
			dQuest.SetActive(false);
			dialogActive = false;
			levelManager.RespawnPlayer ();
		}
